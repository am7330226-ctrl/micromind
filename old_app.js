...
const DEFAULT_HABITS = [
    { id: 'habit-1', emoji: '≡ƒÆº', label: 'Drink Water', done: false },
    { id: 'habit-2', emoji: '≡ƒÜ╢', label: 'Take a Walk', done: false },
    { id: 'habit-3', emoji: '≡ƒôû', label: 'Read 10 Min', done: false },
    { id: 'habit-4', emoji: '≡ƒºÿ', label: 'Breathe / Meditate', done: false },
    { id: 'habit-5', emoji: '≡ƒô╡', label: 'No Phone 1hr', done: false },
    { id: 'habit-6', emoji: '≡ƒîÖ', label: 'Sleep by 11pm', done: false },
];
...
function showToast(message, icon = 'Γ£à') {
...
function buildTaskEl(task) {
    ...
    // AI badge ΓÇö shows sorting state on new inbox tasks
    if (task.aiSorting) {
        const badge = document.createElement('span');
        badge.className = 'ai-badge sorting';
        badge.id = `ai-badge-${task.id}`;
        badge.textContent = 'Γ£¿ AI sortingΓÇª';
...
function updateProgressRing() {
    ...
    const circumference = 2 * Math.PI * 24; // r=24 ΓåÆ ~150.8
...
function performDailyReset() {
    ...
    state.tasks.forEach(t => {
        ...
        // Tasks in focus slots ΓÇö check their original quadrant isn't tracked,
    ...
    showToast(
        `Reset complete! ≡ƒÄë ${completed}/${total} tasks done. Streak: ${streak} day${streak !== 1 ? 's' : ''}!`,
        '≡ƒîÖ'
    );
}
...
function initEventListeners() {
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            ...
            saveState();
            renderAll();
            showToast('Cleared completed inbox tasks!', '≡ƒº╣');
        });
    }
...
const RING_CIRCUMFERENCE = 2 * Math.PI * 88; // Γëê 552.9
...
function pomoTimerDone() {
    ...
    if (Notification && Notification.permission === 'granted') {
        ...
        new Notification('MicroMind ≡ƒìà', {
            body: isFocus
    ...
    const msg = pomo.mode === 'focus'
        ? `≡ƒÄë Focus session done! ${pomo.sessions} session${pomo.sessions !== 1 ? 's' : ''} today.`
        : `Γ£à Break over ΓÇö ready to focus again!`;
    showToast(msg, '≡ƒìà');
...
function pomoUpdateDisplay() {
    ...
    if (pomo.running) {
        document.title = `${m}:${s} ΓÇö MicroMind`;
    } else {
...
function pomoLinkTask(taskId, taskText) {
    if (pomo.linkedTaskId === taskId) {
        ...
        pomoTaskLabelEl.innerHTML = 'Γû╢ ' + parseRichText(taskText);
    }
    pomoRenderFocusTaskPicker();
}
    ...
    stars.forEach(star => {
        ...
        star.addEventListener('click', () => {
            ...
            if (state.moodToday > 0) {
                ...
                showToast(`Mood set: ${MOOD_LABELS[state.moodToday]} ${'Γ¡É'.repeat(state.moodToday)}`, '≡ƒÿè');
            }
        });
    });
}
...
function initSettings() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            showToast('Data exported successfully!', '≡ƒôÑ');
        });
    }
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (parsed && typeof parsed === 'object' && Array.isArray(parsed.tasks)) {
                        ...
                        showToast('Data imported successfully!', '≡ƒÜÇ');
                    } else {
...
function initAI() {
    ...
    aiWorker.addEventListener('message', (event) => {
        ...
        if (type === 'progress') {
            ...
            if (label) label.textContent = `Downloading AI model (first time only)ΓÇª ${Math.round(progress)}%`;
        }
        ...
        if (type === 'result' && taskId) {
            ...
            if (score >= CONFIDENCE_THRESHOLD && category) {
                ...
                saveState();
                renderAll();
                showToast(`≡ƒñû Sorted "${task.text.substring(0, 30)}" ΓåÆ ${CATEGORY_NAMES[category]}`, 'Γ£¿');
            } else {
                // Not confident enough ΓÇö leave in Inbox, show a subtle badge
                saveState();
                renderAll();
            }
        }
...
// ============================================================
// Boot ΓÇö placed after all module code so const declarations are initialized
// ============================================================
