...
function AppContent({
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-main, #f4f6fc)', transition: 'background-color 0.2s ease' }}>
      {/* Sidebar */}
      <Sidebar
        isOpen={sidebarOpen}
        ...
        onNewEntry={() => {
          ...
          document.getElementById('section-brain-dump')?.scrollIntoView({ behavior: 'smooth' });
          setTimeout(() => {
      ...
      {/* Main Content Area */}
      <div className="main-with-sidebar" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100%' }}>
        <Header
          userName={userName}
          onLogout={onLogout}
          showToast={showToast}
        ...
        <main style={{ padding: '32px 24px', maxWidth: '1400px', margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
          ...
          <div id="section-dashboard" style={{
            ...
            gap: '20px', marginBottom: '32px', width: '100%', position: 'relative'
          }}>
            <div>
              <h2 style={{ fontSize: '28px', fontWeight: '700', fontFamily: 'Outfit, sans-serif', color: 'var(--text-primary, #1e293b)', margin: '0 0 6px 0', lineHeight: '1.2' }}>
                {greeting}, {firstName}! 🧠
              </h2>
              <p style={{ fontSize: '15px', color: 'var(--text-secondary, #64748b)', margin: 0, lineHeight: '1.4' }}>
                {urgentCount > 0
                  ? <>You have <span style={{ color: '#a04100', fontWeight: '700' }}>{urgentCount} urgent item{urgentCount !== 1 ? 's' : ''}</span> in Do First quadrant.</>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              <div style={{
                backgroundColor: 'var(--bg-card, #ffffff)', border: '1px solid var(--border-color, #e2e8f0)',
                borderRadius: '16px', padding: '10px 18px', display: 'flex', alignItems: 'center', gap: '12px',
                boxShadow: '0 2px 8px rgba(0,0,0,0.03)'
              }}>
                <span style={{ fontSize: '24px' }}>🔥</span>
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--text-secondary, #64748b)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>Current Streak</div>
                  <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary, #1e293b)', lineHeight: '1.2' }}>{streak}-Day Streak</div>
                </div>
              </div>

              <div style={{
                backgroundColor: '#630ed4', color: '#ffffff',
                borderRadius: '16px', padding: '10px 18px', display: 'flex', alignItems: 'center', gap: '12px',
                boxShadow: '0 4px 16px rgba(99,14,212,0.25)'
              }}>
                <span className="material-symbols-outlined" style={{ fontSize: '22px', fontVariationSettings: "'FILL' 1" }}>military_tech</span>
                <div>
                  <div style={{ fontSize: '10px', color: '#eaddff', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>Level Progress</div>
                  <div style={{ fontSize: '16px', fontWeight: '700', lineHeight: '1.2' }}>Lvl {level} ({xp} XP)</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div id="section-focus-league" className="md:col-span-4 md:row-span-2">
              <FocusLeague />
            </div>
