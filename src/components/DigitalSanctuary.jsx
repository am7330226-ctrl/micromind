...
export default function DigitalSanctuary({ onOpenPomodoro, showToast }) {
  ...
  function handleSound(id) {
    ...
    if (showToast) showToast(`Playing ${SOUNDS.find(s => s.id === id)?.label}`, '🎧');
  }
  ...
  const NowPlayingArt = () => (
    <svg viewBox="0 0 48 48" style={{ width: '100%', height: '100%', borderRadius: '12px' }} xmlns="http://www.w3.org/2000/svg">
      <defs>
  ...
  return (
    <div className="bento-card" style={{ height: '500px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', flexShrink: 0 }}>
        <span className="material-symbols-outlined" style={{ color: '#630ed4', fontSize: '24px' }}>headset</span>
        ...
        {onOpenPomodoro && (
          <button
            ...
            style={{
              ...
              fontSize: '12px', fontWeight: '600', color: '#630ed4', backgroundColor: 'rgba(234, 221, 255, 0.5)',
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', overflow: 'hidden' }}>
        {/* Mood Selector */}
        <div>
          <p style={{ fontSize: '12px', fontWeight: '700', color: '#4a4455', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '16px' }}>
            Current Mood
          </p>
            ...
            {MOODS.map(m => {
              ...
              return (
                <button
                  ...
                  style={{
                    flex: 1, minHeight: '90px', borderRadius: '20px', display: 'flex',
                    ...
                    border: isSelected ? 'none' : '1px solid rgba(204, 195, 216, 0.4)',
                    boxShadow: isSelected ? '0 8px 20px rgba(99, 14, 212, 0.3)' : 'none',
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: '10px 14px', borderRadius: '16px', cursor: 'pointer', transition: 'all 0.2s',
                    backgroundColor: isCurrentlyPlaying ? 'rgba(234, 221, 255, 0.5)' : '#f8fafc',
                    border: isCurrentlyPlaying ? '1px solid #630ed4' : '1px solid rgba(204, 195, 216, 0.3)'
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{
                      width: '36px', height: '36px', borderRadius: '50%',
                      backgroundColor: isCurrentlyPlaying ? '#630ed4' : 'rgba(74, 68, 85, 0.08)',
                    ...
                    <div>
                      <p style={{ fontSize: '14px', fontWeight: '600', color: '#111c2d', margin: 0 }}>{s.label}</p>
                      {isCurrentlyPlaying && <p style={{ fontSize: '10px', color: '#630ed4', fontWeight: '700', textTransform: 'uppercase', margin: '2px 0 0 0' }}>🔊 Playing</p>}
                      {!isCurrentlyPlaying && <p style={{ fontSize: '11px', color: '#4a4455', opacity: 0.7, margin: '2px 0 0 0' }}>{s.desc}</p>}
                    </div>
                  </div>
                  {isSelectedTrack ? (
                    <input
                    ...
                    <span className="material-symbols-outlined" style={{ color: '#4a4455', opacity: 0.5 }}>play_arrow</span>
                  )}
                </div>
              );
            })}
      ...
      {/* Now Playing Bar */}
      <div style={{
        marginTop: '16px', paddingTop: '16px', borderTop: '1px solid rgba(204, 195, 216, 0.3)',
        display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0
      }}>
        <div style={{ width: '48px', height: '48px', borderRadius: '12px', flexShrink: 0, overflow: 'hidden' }}>
          <NowPlayingArt />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: '11px', fontWeight: '700', color: '#4a4455', margin: '0 0 2px 0' }}>
            Current Atmosphere
          </p>
          <p style={{ fontSize: '14px', fontWeight: '600', color: '#111c2d', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {activeSound.label} · {isPlaying ? 'Playing Ambient Synthesis' : 'Paused'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
          <button
            ...
            style={{
              width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#630ed4', color: 'white',
