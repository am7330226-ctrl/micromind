...
function getCellColor(count) {
  if (!count || count === 0) return 'var(--heatmap-empty, rgba(139,92,246,0.06))';
...
export default function HabitHeatmap() {
  ...
  const scoreMap = useMemo(() => {
    const map = {};
    history.forEach(h => { map[h.date] = h.tasksCompleted || 0; });
    return map;
  }, [history]);
  ...
  return (
    <div className="heatmap-wrapper">
      <div className="heatmap-meta">
        ...
        <span className="heatmap-subtitle">{totalDone} tasks completed · {activeDays} active days</span>
      </div>
      ...
      <div className="heatmap-scroll">
        ...
        {/* Grid */}
        <div className="heatmap-grid" role="grid" aria-label="84-day activity heatmap">
          {/* Month header row */}
      ...
      <div className="heatmap-legend" aria-hidden="true">
        ...
        {[0, 1, 2, 4, 7, 10].map(n => (
          <div key={n} className="heatmap-legend-cell" style={{ background: getCellColor(n) }} />
        ))}
      ...
      {tooltip && (
        ...
        >
          ...
          <span>{tooltip.count > 0 ? `${tooltip.count} task${tooltip.count === 1 ? '' : 's'} completed` : 'No activity'}</span>
        </div>
      )}
    </div>
  );
