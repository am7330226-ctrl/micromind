...
function drawDoughnut(canvas, history) {
  ...
  segs.forEach((seg) => {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(seg.label, lX + 12, lY + 1);
    ctx.fillStyle = CHART_COLORS.textMuted;
...
export default function AnalyticsDashboard({ open, onClose }) {
  ...
  const best =
    history.length > 0
      ? history.reduce((a, b) =>
          (a.tasksCompleted || 0) >= (b.tasksCompleted || 0) ? a : b,
        )
      : null;
  ...
  return (
    <>
      <div
        className="analytics-backdrop visible"
        id="analytics-backdrop"
        onClick={onClose}
      />
      <div
        id="analytics-panel"
        className="analytics-panel open"
        role="dialog"
        aria-modal="true"
        ...
        <div className="analytics-inner">
          ...
          >
            ...
            <div className="analytics-stat-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span
                  className="analytics-stat-val"
                  style={{ fontSize: '16px' }}
                >
                  {best
                    ? new Date(best.date + 'T00:00:00').toLocaleDateString(
                        'en-US',
          ...
          >
            ...
            <div className="badges-grid">
              <div
                className={`badge-item ${state.badges?.includes('task-crusher') ? '' : 'locked'}`}
                title="Complete 10 tasks in a single day"
              >
                <span className="badge-icon">🗡️</span>
                <span className="badge-name">Task Crusher</span>
              </div>
              <div
                className={`badge-item ${state.badges?.includes('7-day-warrior') ? '' : 'locked'}`}
              ...
              <div
                className={`badge-item ${state.badges?.includes('focus-master') ? '' : 'locked'}`}
                title="Log 5 Pomodoro sessions"
              >
