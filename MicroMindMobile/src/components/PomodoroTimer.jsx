...
import { toggleAmbientSound, getCurrentSoundState } from '../services/soundEngine';
...
export default function PomodoroTimer() {
  ...
  useEffect(() => {
    ...
    if (isRunning && timeLeft > 0) {
      ...
      if (selectedPreset.label === 'Pomodoro') {
        dispatch({ type: 'SET_POMODORO_SESSIONS', sessions: (pomodoroSessions || 0) + 1 });
      }
    }
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {TIMER_PRESETS.map(preset => (
          <Pressable
            ...
            style={[
              styles.presetChip,
              {
                backgroundColor: selectedPreset.label === preset.label ? colors.primaryViolet : colors.bgCard,
                borderColor: selectedPreset.label === preset.label ? colors.primaryViolet : colors.borderColor,
              },
            ]}
          ...
          >
            <Text
              style={[
                ...
                { color: selectedPreset.label === preset.label ? '#ffffff' : colors.textSecondary },
              ]}
            >
              {preset.label}
            </Text>
      ...
      <View style={[styles.timerCard, { backgroundColor: colors.bgCard, borderColor: colors.borderColor }]}>
        <Text style={[styles.timerDisplay, { color: colors.textPrimary }]}>{formattedTime}</Text>
        ...
        <View style={styles.controlsRow}>
          <Pressable
            style={[styles.primaryButton, { backgroundColor: colors.primaryViolet }]}
            onPress={handleStartPause}
          >
            <Ionicons name={isRunning ? 'pause' : 'play'} size={24} color="#ffffff" />
            <Text style={styles.buttonText}>{isRunning ? 'Pause' : 'Start Focus'}</Text>
          </Pressable>

          <Pressable
            style={[styles.secondaryButton, { backgroundColor: 'rgba(255,255,255,0.08)' }]}
            onPress={handleReset}
          >
        ...
        {SOUND_TRACKS.map(track => {
          ...
          return (
            ...
            >
              ...
              <Text style={[styles.soundLabel, { color: isActive ? colors.primaryViolet : colors.textPrimary }]}>
                {track.label}
              </Text>
              <Ionicons
